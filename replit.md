...
- **Arrowverse Mode** — `src/pages/ArrowverseMode.tsx` — CW episodic story engine with 7 CW show selector (Arrow, The Flash, Supergirl, Legends of Tomorrow, Batwoman, Black Lightning, Superman & Lois), each with unique tone/city/cast/homeBase woven into the prompt; **Cold Open Format** — CW episode structure (COLD OPEN → ★ ACT ONE ★ → ★ ACT TWO ★ → ★ ACT THREE ★ → ── TO BE CONTINUED ──) toggled in ConfigurePanel; **Crossover Event Mode** — picks two heroines from different shows with friction/dynamics explicitly baked into the prompt; **Season Big Bad Tracker** — declare any villain as the season villain via a "DECLARE BIG BAD" button, tracks episode appearances across the season in localStorage (`sw_arrowverse_bigbad_v1`), shown as an eye-icon tracker in the ConfigurePanel and EpisodeList; **Season Finale Unlock** — after 8 completed episodes the final episode card glows gold with a "★ SEASON FINALE UNLOCKED ★" banner and launches with a SEASON FINALE directive; archive titles include S{N}E{N} numbering + [CROSSOVER]/[SEASON FINALE] tags; all features require no backend changes (all context packed into the `details` field of `/api/story/superhero`)
