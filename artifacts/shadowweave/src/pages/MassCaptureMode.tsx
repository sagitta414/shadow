...
export default function MassCaptureMode({ onBack }: Props) {
  ...
  useEffect(() => {
    ...
    fetch("/api/story/psyche-update", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ chapterText: _ch.slice(0, 2500), heroineName: allHeroines.join(" & ") || undefined, currentSanity: _s, currentResistance: _r }) })
  ...
  if (step === 3 && chapters.length > 0) {
    return (
      <div style={{ minHeight: "100vh", padding: "2rem", maxWidth: "860px", margin: "0 auto" }}>
        ...
        {psycheLog.length > 0 && <PsycheMeter sanity={psycheSanity} resistance={psycheResistance} log={psycheLog} heroineName={allHeroines} />}
        {chapters.map((ch, i) => (
          <div key={i}>
