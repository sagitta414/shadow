...
export default function RescueGoneWrongMode({ onBack }: Props) {
  ...
  useEffect(() => {
    ...
    fetch("/api/story/psyche-update", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ chapterText: _ch.slice(0, 2500), heroineName: fCaptive || undefined, currentSanity: _s, currentResistance: _r }) })
  ...
  if (step === 3) {
    ...
    return (
      <div style={{ minHeight: "100vh", padding: "2rem", maxWidth: "860px", margin: "0 auto" }}>
        ...
        {psycheLog.length > 0 && <PsycheMeter sanity={psycheSanity} resistance={psycheResistance} log={psycheLog} heroineName={fCaptive || undefined} />}
        {chapters.map((ch, i) => (
          <div key={i}>
    ...
    <div style={{ minHeight: "100vh", padding: "2rem", maxWidth: "900px", margin: "0 auto" }}>
      ...
      <div className="mode-dual-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "2rem" }}>
        ...
        <Sec title="RESCUER HEROINE (WHO FAILS)" acc={acc} rgb={accRgb}>
          <HeroinePicker value={rescuer || customRescuer} onChange={name => { setRescuer(name); setCustomRescuer(""); }} accentColor={acc} accentRgb={accRgb} />
        </Sec>
      </div>
