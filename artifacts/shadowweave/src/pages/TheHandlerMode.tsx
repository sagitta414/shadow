...
export default function TheHandlerMode({ onBack }: Props) {
  ...
  const [psychologicalAnchoring, setPsychologicalAnchoring] = useState("");
  const [complianceDocumentation, setComplianceDocumentation] = useState("");
  ...
  async function generate(isFirst: boolean) {
    ...
    try {
      ...
      const _complianceTrackingMap: Record<string,string> = {"logged":"Logged & Filed","verbal":"Verbal Reporting","inspection":"Physical Inspection","behavioral":"Behavioral Observation"}; return [handlerTone ? `Handler Tone: ${_handlerToneMap[handlerTone] ?? handlerTone}` : "", complianceTracking ? `Compliance Tracking: ${_complianceTrackingMap[complianceTracking] ?? complianceTracking}` : "", psychologicalAnchoring ? `Psychological Anchoring Technique (how the handler builds her dependency): ${psychologicalAnchoring.replace(/_/g," ")}` : "", complianceDocumentation ? `Compliance Documentation Method: ${complianceDocumentation.replace(/_/g," ")}` : ""].filter(Boolean).join("\n"); })(), handlerTone, complianceTracking, psychologicalAnchoring, complianceDocumentation, heroine: fH, handlerType, handlerName: handlerDisplayName, handlerDesc: fHT?.desc ?? "", facility, protocol, chapters: isFirst ? [] : chapters, sessionNumber: isFirst ? 1 : chapters.length + 1, storyLength, continueDir }),
      });
    ...
    return (
      <div style={{ minHeight: "100vh", padding: "2rem", maxWidth: "740px", margin: "0 auto" }}>
        ...
        <div style={{marginTop:"1.25rem"}}>
          ...
          <div style={{marginBottom:"0.875rem"}}>
            <div style={{fontSize:"0.57rem",fontFamily:"'Montserrat',sans-serif",fontWeight:700,letterSpacing:"2.5px",textTransform:"uppercase",color:"rgba(200,195,240,0.35)",marginBottom:"0.5rem"}}>PSYCHOLOGICAL ANCHORING</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:"0.4rem"}}>
              {[{id:"permission_system",icon:"📅",label:"Permission System — She Requests Everything"},{id:"enforced_gratitude",icon:"🙏",label:"Enforced Gratitude Rituals"},{id:"name_replacement",icon:"🏷️",label:"Name Replacement — New Identity Assigned"},{id:"controlled_information",icon:"📵",label:"Controlled Information — He Decides What She Knows"},{id:"manufactured_warmth",icon:"🪡",label:"Manufactured Affection — He Is Her Only Warmth"},{id:"cyclical_conditioning",icon:"🔄",label:"Cyclical Punishment and Reward"}].map((opt:{id:string;icon:string;label:string}) => (<button key={opt.id} onClick={() => setPsychologicalAnchoring(psychologicalAnchoring === opt.id ? "" : opt.id)} style={{display:"flex",alignItems:"center",gap:"0.35rem",padding:"0.4rem 0.8rem",borderRadius:"20px",border:`1px solid ${psychologicalAnchoring === opt.id ? "#34D399" : "rgba(200,195,240,0.15)"}`,background:psychologicalAnchoring === opt.id ? "rgba(52,211,153,0.16)" : "rgba(255,255,255,0.03)",color:psychologicalAnchoring === opt.id ? "#34D399" : "rgba(200,195,240,0.55)",fontSize:"0.7rem",fontFamily:"'Montserrat',sans-serif",fontWeight:psychologicalAnchoring === opt.id ? 700:400,cursor:"pointer",transition:"all 0.18s",minHeight:"36px"}}><span>{opt.icon}</span><span>{opt.label}</span></button>))}
            </div>
          </div>
          <div style={{marginBottom:"0"}}>
            <div style={{fontSize:"0.57rem",fontFamily:"'Montserrat',sans-serif",fontWeight:700,letterSpacing:"2.5px",textTransform:"uppercase",color:"rgba(200,195,240,0.35)",marginBottom:"0.5rem"}}>COMPLIANCE DOCUMENTATION</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:"0.4rem"}}>
              {[{id:"daily_report",icon:"📋",label:"Daily Compliance Report"},{id:"behavioral_chart",icon:"📈",label:"Behavioral Progress Chart"},{id:"recorded_confessions",icon:"🎙️",label:"Recorded Confessions — Her Own Voice"},{id:"witnessed_declarations",icon:"📜",label:"Witnessed Declarations — Signed"},{id:"progress_photographs",icon:"📷",label:"Progress Photographs — Catalogued"},{id:"incident_logs",icon:"🗒️",label:"Incident Logs — Every Resistance Noted"}].map((opt:{id:string;icon:string;label:string}) => (<button key={opt.id} onClick={() => setComplianceDocumentation(complianceDocumentation === opt.id ? "" : opt.id)} style={{display:"flex",alignItems:"center",gap:"0.35rem",padding:"0.4rem 0.8rem",borderRadius:"20px",border:`1px solid ${complianceDocumentation === opt.id ? "#34D399" : "rgba(200,195,240,0.15)"}`,background:complianceDocumentation === opt.id ? "rgba(52,211,153,0.16)" : "rgba(255,255,255,0.03)",color:complianceDocumentation === opt.id ? "#34D399" : "rgba(200,195,240,0.55)",fontSize:"0.7rem",fontFamily:"'Montserrat',sans-serif",fontWeight:complianceDocumentation === opt.id ? 700:400,cursor:"pointer",transition:"all 0.18s",minHeight:"36px"}}><span>{opt.icon}</span><span>{opt.label}</span></button>))}
            </div>
          </div>
          </div>
        </div>
