...
export default function DualCaptureMode({ onBack }: Props) {
  ...
  useEffect(() => {
    ...
    fetch("/api/story/psyche-update", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ chapterText: _ch.slice(0, 2500), heroineName: [fH1, fH2].filter(Boolean).join(" & ") || undefined, currentSanity: _s, currentResistance: _r }) })
  ...
  if (step === 3) {
    ...
    return (
      <div style={{ minHeight: "100vh", padding: "2rem", maxWidth: "860px", margin: "0 auto" }}>
        ...
        {psycheLog.length > 0 && <PsycheMeter sanity={psycheSanity} resistance={psycheResistance} log={psycheLog} heroineName={[fH1, fH2].filter(Boolean)} />}
        {chapters.map((ch, i) => (
          <div key={i}>
